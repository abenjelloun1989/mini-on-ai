...
- Uses `meta["gumroad_description"]` (rich HTML sales copy generated by Claude)
...
- Stored in `meta.json` as `gumroad_description` (HTML: hook, personas, what's inside, CTA)
- Used in Telegram publish notification (tap-to-copy) and Gumroad API updates
...
- Category distribution enforced: max 4 prompt-packs per 10-idea batch
